...
## Server Delivery Model: Ports, Adapters, and Tests

The Ktor server follows hexagonal architecture (also called Ports & Adapters). This keeps product
rules easy to test and stops framework, database, and provider details from spreading into every
feature.

- **Domain** contains Hearcho concepts and rules: immutable models, value objects, and sealed
  failures. It must not import Ktor, Exposed, Redis, RabbitMQ, or LiveKit.
- **Application** contains one use case per product action, such as `RegisterUser`, `JoinRoom`, or
  `IssueMediaCredentials`. A use case depends on small project-owned interfaces called **ports**.
- **Inbound adapters** translate an external request into a use-case call. Ktor HTTP routes,
  WebSocket handlers, scheduled jobs, and provider webhooks are inbound adapters. They validate
  transport input, map outcomes to transport responses, and contain no product decision-making.
- **Outbound adapters** implement application ports. Exposed/PostgreSQL repositories, Redis presence
  storage, RabbitMQ publishing, password hashing, clocks, and LiveKit token signing are outbound
  adapters.
- **Bootstrap** is the only composition root. Its Koin modules create adapters, inject them into use
  cases, and install Ktor plugins and routes. No feature package should construct its own database,
  HTTP client, or provider SDK object.

Use this package shape for each server feature; create only the folders that the feature needs:

```text
server/src/main/kotlin/dev/kavrin/hearcho/
├── bootstrap/                         # Configuration and dependency wiring
├── <feature>/domain/                  # Pure product rules and models
├── <feature>/application/             # Use cases and ports
│   └── port/                           # Inbound use-case interfaces and outbound dependencies
└── <feature>/adapter/
    ├── in/http/ or in/websocket/      # Ktor request/response translation
    └── out/                            # Exposed, Redis, RabbitMQ, LiveKit, and other integrations
```

Testing follows the same boundary. Start with a fast use-case unit test using hand-written fakes;
then add an adapter integration test only when a real integration is introduced; finally add a small
Ktor HTTP or WebSocket test for the public contract. Do not mock Ktor, Exposed, or LiveKit
internals. Prefer fakes for ports and Testcontainers for PostgreSQL-dependent behavior.
...
- [x] Task 0.F1: Confirm architecture and decisions.
    ...
    - [x] Create `core`, `app/sharedLogic`, `app/sharedUI`, `app/androidApp`, `app/iosApp`, and
      `server` boundaries.
...
- [x] Task 0.S1: Establish Ktor bootstrap and configuration.
    - [x] Move Ktor bootstrap into an `Application.module` function.
    - [x] Add a typed environment configuration loader.
    - [x] Fail startup when required configuration is absent.
    - [x] Keep secrets out of committed configuration files.

- [x] Task 0.S2: Expose server liveness.
    - [x] Add a `/health/live` route.
    - [x] Add a Ktor test for `/health/live`.

- [x] Task 0.S3: Establish the server ports-and-adapters and test baseline.
    - [x] Add the `bootstrap`, feature `domain`, `application`, and `adapter` package conventions
      described above.
    - [x] Keep Ktor plugin installation, route registration, configuration loading, and dependency
      wiring in `bootstrap`.
    - [x] Add a test-fixture convention with hand-written fakes for application ports; do not add a
      mocking framework by default.
    - [x] Add one pure Kotlin use-case test that runs without Ktor or external services.
    - [x] Add one Ktor `testApplication` test that proves an inbound adapter is wired to its use
      case.
    - [x] Document the three test levels in server test package README or KDoc: use-case unit,
      adapter integration, and transport contract.

- [ ] Task 0.S4: Establish Koin server composition.
    - [ ] Add Koin core and the Ktor integration to the `server` module.
    - [ ] Define server Koin modules under `bootstrap` for configuration, adapters, use cases, and
      route registration.
    - [ ] Bind project-owned port interfaces to their concrete adapters; bind use cases as factories
      or singletons according to their state and lifecycle needs.
    - [ ] Start and stop Koin through the Ktor application lifecycle; do not call global `startKoin`
      from feature code or tests.
    - [ ] Resolve dependencies while installing routes or constructing adapters, never with `get()`
      inside a route handler or use case.
    - [ ] Add a Ktor `testApplication` test that loads a test Koin module with fake port bindings
      and verifies the application starts.
    - [ ] Add a test proving the production Koin module verifies without constructing
...
- [ ] Task 0.C1: Replace generated shared placeholders.
    ...
    - [ ] Add Android and iOS `Clock` implementations only if the common implementation is
      insufficient.
...
- [ ] Task 1.F4: Establish typed domain outcomes and validation.
    ...
    - [ ] Define a project result alias based on `Either<DomainFailure, A>` where it improves public
      domain APIs.
    ...
    - [ ] Add tests proving `NonEmptyList` validation accumulation preserves deterministic error
      order.
...
- [ ] Task 1.S1: Install the PostgreSQL persistence stack.
    ...
    - [ ] Keep these dependencies inside `bootstrap` and `adapter.out.persistence`; do not expose
      Exposed types through domain models, ports, or use cases.

- [ ] Task 1.S2: Configure database startup.
    - [ ] Create a Hikari data-source factory.
    - [ ] Add a Flyway startup migration runner.
    - [ ] Wire the data source and migration runner only in `bootstrap`.
    - [ ] Register the data source, migration runner, and persistence adapters through the server
      Koin modules; application use cases continue to receive only ports.
...
- [ ] Task 1.S4: Define persistence output ports and Exposed adapters.
    - [ ] Define only the project-owned persistence ports required by the first use cases; avoid a
      generic CRUD repository base class.
    - [ ] Keep port inputs and results in domain/application types, never `ResultRow`, `Table`, or
      JDBC types.
    - [ ] Add Exposed table mappings and repository adapters for each migration under
      `adapter.out.persistence`.
    - [ ] Add a transaction executor inside the persistence adapter that runs blocking database work
      on an IO dispatcher.

- [ ] Task 1.S5: Verify the persistence adapter boundary.
    - [ ] Add PostgreSQL Testcontainers dependency and shared database-test setup.
    - [ ] Add a migration integration test against PostgreSQL.
    - [ ] Add an Exposed adapter integration test that inserts and reads a user through a
      persistence port.
    - [ ] Add a use-case unit test that uses an in-memory fake persistence port and needs neither
      PostgreSQL nor Ktor.
    - [ ] Verify no application or domain source imports Exposed, Flyway, HikariCP, or JDBC
      packages.
...
- [ ] Task 1.C4: Establish safe parallel composition with `parZip`.
    ...
    - [ ] Combine parallel results into a typed `Either` or `Raise` workflow without throwing for
      expected failures.
...
- [ ] Task 2.S1: Configure Ktor authentication infrastructure.
    ...
    - [ ] Install plugins and framework configuration in `bootstrap`; keep them out of auth use
      cases.

- [ ] Task 2.S2: Define authentication ports and adapters.
    - [ ] Define narrow output ports for user lookup/write, session lifecycle, password hashing,
      token issuance, and time.
    - [ ] Implement Exposed user and session persistence adapters behind those ports.
    - [ ] Implement the Argon2id password-hash adapter behind a project-owned password port.
    - [ ] Implement the JWT signer/verifier adapter behind project-owned token ports.
    - [ ] Inject a clock port so expiry behavior has deterministic tests.

- [ ] Task 2.S3: Implement authentication use cases.
    ...
    - [ ] Return project-owned outcomes; routes map them to HTTP responses.

- [ ] Task 2.S4: Expose authentication through Ktor inbound adapters.
    - [ ] Add `/api/v1/auth/register`.
    - [ ] Add `/api/v1/auth/sign-in`.
    - [ ] Add `/api/v1/auth/refresh`.
    - [ ] Add `/api/v1/auth/sign-out`.
    - [ ] Keep request/response DTOs and HTTP status mapping in `adapter.in.http`; do not return
      Ktor responses from use cases.

- [ ] Task 2.S5: Verify and protect authentication.
    - [ ] Add use-case unit tests with fake ports for registration, duplicate email, sign-in,
      invalid credentials, expiry, rotation, and revocation.
    - [ ] Add adapter integration tests for Exposed persistence, Argon2id hashing, and JWT
      verification where their behavior matters.
    - [ ] Add Ktor route-contract tests for every success case.
    - [ ] Add Ktor route-contract tests for every validation and authorization failure.
...
- [ ] Task 2.E1: Complete Phase 2 exit criteria.
    ...
    - [ ] Auth tests cover invalid input, duplicate account, invalid credentials, expiry, rotation,
      and revocation.
...
- [ ] Task 3.S1: Define room ports and persistence adapters.
    - [ ] Define ports for room persistence, participant membership, pagination, identity, and time
      from the needs of the room use cases.
    - [ ] Implement the Exposed room adapter for create, active cursor pagination, and lookup by ID.
    - [ ] Implement the Exposed participant adapter for idempotent join and leave.
    - [ ] Keep concurrency, unique-constraint, and transaction details inside the persistence
      adapter.

- [ ] Task 3.S2: Implement room application use cases.
    - [ ] Implement authorization rules as application/domain policy, not as route conditionals.
    - [ ] Implement `CreateRoom`.
    - [ ] Implement `ListActiveRooms`.
    - [ ] Implement `GetRoomDetails`.
    - [ ] Implement `JoinRoom`.
    - [ ] Implement `LeaveRoom`.
    - [ ] Implement `EndRoom`.
...
- [ ] Task 3.S4: Verify and protect room operations.
    - [ ] Add use-case unit tests with fakes for room lifecycle and owner/participant authorization
      rules.
    - [ ] Add PostgreSQL adapter integration tests for cursor pagination and idempotent membership
      writes.
    - [ ] Add Ktor route-contract tests for owner and participant permissions.
    - [ ] Add concurrent-join integration test against PostgreSQL.
...
- [ ] Task 3.E1: Complete Phase 3 exit criteria.
    - [ ] An authenticated user can create, list, inspect, join, leave, and end a room on both
      clients.
...
- [ ] Task 4.S1: Establish the WebSocket gateway.
    ...
    - [ ] Treat the gateway as an inbound adapter: translate frames into use-case inputs and map
      outcomes to project-owned event contracts.

- [ ] Task 4.S2: Synchronize room state.
    - [ ] Define application ports for connection delivery, sequence allocation, room snapshot
      lookup, and membership authorization.
...
- [ ] Task 4.S3: Implement reconnect and verify gateway behavior.
    ...
    - [ ] Add use-case unit tests with fake delivery and snapshot ports for authorization, ordering,
...
- [ ] Task 4.S4: Persist ephemeral presence in Redis.
    ...
    - [ ] Implement Redis behind a presence port.
    - [ ] Add Redis Testcontainer adapter tests for TTL, disconnect cleanup, and unavailable-Redis
      behavior.
...
- [ ] Task 5.S3: Authorize and expose media credentials.
    ...
    - [ ] Implement the authorization and credential-issuance workflow as an application use case
      that depends on `VoiceTokenProvider`.
    - [ ] Add provider-neutral `POST /api/v1/rooms/{roomId}/media-credentials`.
    - [ ] Add use-case unit tests with a fake `VoiceTokenProvider` for membership, role, and expiry
      decisions.
    - [ ] Add LiveKit adapter tests for token claim mapping and expiry.
    - [ ] Add Ktor route-contract authorization tests.
...
- [ ] Task 5.S4: Adapt provider webhooks.
    ...
    - [ ] Add adapter tests for signature rejection and provider-payload mapping without calling
      application use cases directly from provider DTOs.
...
- [ ] Task 5.E1: Complete Phase 5 exit criteria.
    ...
    - [ ] Shared client and Ktor room-service tests pass with fake voice providers and no LiveKit
      runtime.
...
- [ ] Task 6.S1: Implement moderator commands.
    - [ ] Define moderation application ports before adding delivery or persistence details.
...
- [ ] Task 6.S2: Persist and enforce safety decisions.
    ...
    - [ ] Implement report, block, and audit persistence as outbound adapters.
    - [ ] Add use-case tests for each authorization rule and Ktor route-contract tests for each
      moderation action.

- [ ] Task 6.S3: Implement reliable RabbitMQ delivery.
    - [ ] Add RabbitMQ Java client.
    - [ ] Add broker configuration and connection factory.
    - [ ] Define ports for outbox claiming, event publishing, and retry scheduling.
    - [ ] Add outbox repository claim adapter operation.
    ...
    - [ ] Keep RabbitMQ delivery types and client calls inside outbound adapters.
...
- [ ] Task 6.S4: Implement notification consumers.
    ...
    - [ ] Inject FCM and APNs adapters through project-owned notification delivery ports.

- [ ] Task 6.S5: Verify asynchronous delivery.
    - [ ] Add publisher and consumer use-case tests with fake ports for idempotency and retry
      decisions.
    - [ ] Add integration test from database outbox to consumer using PostgreSQL and RabbitMQ
      Testcontainers.
...
- [ ] Task 7.S2: Instrument distributed traces.
    ...
    - [ ] Add spans for auth, room commands, WebSocket sessions, outbox publishing, and token
      issuance.
...
- [ ] Task 7.S6: Enforce architecture and test boundaries in CI.
    - [ ] Add a server architecture test that prevents domain/application packages from importing
      Ktor, Exposed, JDBC, Redis, RabbitMQ, or LiveKit packages.
    - [ ] Add a server architecture test that prevents adapters from being injected into each other
      instead of through application ports.
    - [ ] Run use-case unit tests separately from Testcontainer adapter tests so fast feedback
      remains available.
    - [ ] Run the full server test suite, static analysis, and formatting checks in CI.

- [ ] Task 7.S7: Validate server capacity.
...
- [ ] Task 7.E1: Complete Phase 7 exit criteria.
    ...
    - [ ] Resilience tests cover network loss, dependency loss, process lifecycle, and audio
      interruption.
...
- [ ] Task 8.E1: Complete Phase 8 exit criteria.
    ...
    - [ ] Android and iOS release candidates pass acceptance, accessibility, privacy, and
